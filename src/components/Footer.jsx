...
const Footer = () => {
  ...
  const aboutText = settings?.footer?.aboutText || 'Your one-stop destination for stylish, comfortable & premium quality clothing in Bangladesh.';
  const phone = settings?.supportPhone || '+880 1711-000000';
  const email = settings?.supportEmail || 'support@stylehub.com.bd';
  const address = settings?.storeAddress || 'House 12, Road 5, Dhanmondi, Dhaka-1205, Bangladesh';
  const copyright = settings?.footer?.copyrightText || 'StyleHub Bangladesh. All Rights Reserved.';

  const fbLink = settings?.footer?.facebookUrl || 'https://facebook.com';
  const instaLink = settings?.footer?.instagramUrl || 'https://instagram.com';
  const tiktokLink = settings?.footer?.tiktokUrl || 'https://tiktok.com';
  const ytLink = settings?.footer?.youtubeUrl || 'https://youtube.com';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 pt-2">
              {fbLink && (
                <a href={fbLink} target="_blank" rel="noreferrer" className="p-2 bg-white rounded-full text-slate-700 hover:text-[#ff2056] shadow-xs border border-gray-200 transition-colors">
              ...
              {instaLink && (
                <a href={instaLink} target="_blank" rel="noreferrer" className="p-2 bg-white rounded-full text-slate-700 hover:text-[#ff2056] shadow-xs border border-gray-200 transition-colors">
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
              ...
              {tiktokLink && (
                <a href={tiktokLink} target="_blank" rel="noreferrer" className="p-2 bg-white rounded-full text-slate-700 hover:text-[#ff2056] shadow-xs border border-gray-200 transition-colors">
                  <span className="text-xs font-black">Tk</span>
                </a>
              )}
              {ytLink && (
                <a href={ytLink} target="_blank" rel="noreferrer" className="p-2 bg-white rounded-full text-slate-700 hover:text-[#ff2056] shadow-xs border border-gray-200 transition-colors">
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
