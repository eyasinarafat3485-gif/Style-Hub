...
import { Mail, Lock, Eye, EyeOff, ArrowRight, ShieldCheck, Sparkles, CheckCircle2, AlertCircle } from 'lucide-react';
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      const errorText = res.error || 'Login failed. Please check your credentials.';
  ...
  const handleGoogleLoginClick = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const errorText = res.error || 'Google login failed.';
        setErrorMessage(errorText);
        toast.error(errorText);
      }
    } catch (err) {
      setIsSubmitting(false);
      // If user closed the popup or canceled, show clean message
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-stone-50/60 to-white selection:bg-[#ff2056] selection:text-white">
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-xl shadow-gray-200/40 p-6 sm:p-9 space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-serif font-extrabold text-slate-900 tracking-tight">
              Welcome Back<span className="text-[#ff2056]">.</span>
            </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <a href="#forgot" className="text-[11px] font-semibold text-[#ff2056] hover:underline">
                  Forgot password?
                </a>
              </div>
              <div className="relative flex items-center">
            ...
            <div className="flex items-center justify-between pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  ...
                  className="w-4 h-4 accent-[#ff2056] rounded cursor-pointer"
                />
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full py-2.5 sm:py-3 bg-[#ff2056] hover:bg-[#d6103e] active:bg-[#b80830] text-white text-xs sm:text-sm font-bold uppercase tracking-wider rounded-lg shadow-lg shadow-rose-500/20 transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
            >
              {isSubmitting ? (
